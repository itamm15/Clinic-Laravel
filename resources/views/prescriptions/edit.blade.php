  ...
  <form action="{{ route('prescriptions.update', $prescription->id) }}" method="POST">
    ...
    <div class="row">
      ...
      <div class="col-md-6 mb-3">
        ...
        <select name="visit_id" id="visit_id" class="form-select">
          ...
          @foreach ($visits as $visit)
            <option value="{{ $visit->id }}" {{ old('visit_id', $prescription->visit_id) == $visit->id ? 'selected' : '' }}>
      ...
      <div class="col-md-6 mb-3">
        ...
        <input type="date" name="date" id="date" class="form-control" value="{{ old('date', $prescription->date) }}" required>
      </div>

      <div class="col-md-12 mb-3">
        <label for="description" class="form-label">Opis</label>
        <input type="text" name="description" id="description" class="form-control" value="{{ old('description', $prescription->description) }}" required>
      </div>
    </div>
